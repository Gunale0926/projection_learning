...
Assume $W_tK=V$, let $\Pi=KK^+$, $z=(I-\Pi)k$, and $e=v-W_tk$. Let $W_t+\Delta$ preserve old constraints, so $\Delta K=0$. If $z\ne0$ and
\begin{align*}
\norm{v-(W_t+\Delta)k}\le\gamma\norm{e}
\end{align*}
for $0\le\gamma<1$, then
\begin{align*}
...
Assume $z\ne0$ and fix $0\le\gamma\le1$. Among all updates satisfying
\begin{align*}
...
Let $(k_t,v_t)_{t=1}^T$ be compatible evidence, and let $K_t=[k_1,\ldots,k_t]$ and $V_t=[v_1,\ldots,v_t]$. Initialize $W_1=W_0$. At step $t$, let $\Pi_{t-1}=K_{t-1}K_{t-1}^+$ and $z_t=(I-\Pi_{t-1})k_t$. If $z_t=0$, make no update. If $z_t\ne0$, apply hard CPM:
\begin{align*}
