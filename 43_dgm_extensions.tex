...
\end{align*}
For a block of length $L$, the DGM state history need not be stored for backpropagation; the additional activation memory is the selected read computation, not the full sequence of memory states. The DGM-LM mixer replaces the $\abs{N}dr$ adapter term by sparse concept lookup, edge-gated value aggregation, and detached graph-write costs.
\end{theorem}
