\section{CGA Implementation Extensions}
\label{app:dgm-extensions}

This appendix collects CGA mechanics and large-model details that are useful for implementations but secondary to the main refine-or-repair argument.
...
\begin{proposition}[Trees as generalized hard-CGAs]
\label{prop:tree-as-dgm:informal}
Given a binary distinction tree, construct one generalized hard-route CGA concept for each leaf and attach the path distinctions as hard eligibility constraints for that concept. If the generalized hard route selects the unique eligible leaf concept and each concept memory equals the tree leaf predictor, then the generalized CGA and the tree make identical predictions.
\end{proposition}
...
\subsection{CGA data-structure cost}
...
\begin{theorem}[CGA data-structure bound]
\label{thm:dgm-complexity:informal}
For exact search over $N_t$ centroids in $\R^d$, one edge-gated CGA prediction costs $O(N_td+Q d+\abs{N}d+C_{\mathrm{local}})$, where $N=N(h)$ and $Q=Q(h)$ is an edge count. One observation that repairs a selected concept costs this plus the guarded local repair scoring cost. One observation that creates a new anchor-centroid pair and $q$ guarded distinction edges costs an additional $O(d+qd)$ plus anchor-order query bookkeeping. With sparse categorical memories of average support $\bar s$, the runtime state uses $O(N_t(2d+\bar s)+\abs{E_t}d)$ memory.
\end{theorem}
...
Anchors, centroids, and edges live in query space. If the encoder or query map changes rapidly while the CGA state persists, old anchors and edges describe stale geometry. Reports should therefore state which training mode is used.

\paragraph{Mode A: frozen encoder.}
Train the backbone and query map first, then freeze $\phi_\theta$, $Q$, and the backbone distribution used for CGA smoothing while running CGA runtime learning. This is the cleanest protocol for the first experiments and is the default interpretation of the theory.
...
During sequential detached training, backpropagation sees the selected read computation, while the CGA state is updated in the same order as streaming inference. The important comparison is with test-time gradient adaptation \citep{sun2020test,wang2021tent,niu2022eata,zhang2022memo,sun2025learning}. A TTT-style layer typically performs an inner backward pass at inference time. A CGA layer performs retrieval, sparse local read, and closed-form or sufficient-statistic repair. The cost is forward-only:
\begin{align*}
\text{CGA inference}=\text{backbone forward}+\text{retrieval}+\text{sparse local repair}.
\end{align*}

\begin{theorem}[CGA layer cost]
\label{thm:dgm-layer-cost:informal}
Consider a CGA adapter extension with hidden dimension $d$, query dimension $d_q$, local rank $r$, candidate count $\abs{N}$, selected incident edge-gate count $Q_N$, and concept-bank query cost $C_{\mathrm{ret}}(N_t,d_q)$. For one token, the read overhead is
\begin{align*}
...
\end{align*}
For a block of length $L$, the CGA state history need not be stored for backpropagation; the additional activation memory is the selected read computation, not the full sequence of memory states. The CGA-LM mixer replaces the $\abs{N}dr$ adapter term by sparse concept lookup, edge-gated value aggregation, and detached graph-write costs.
\end{theorem}
...
Budgeting is necessary for long training. A practical CGA layer maintains $N_t\le N_{\max}$, merges only compatible unprotected concepts, and otherwise evicts or compresses low-score memories. These budget actions are maintenance operations, not learning operations in the formal refine-or-repair sense. The refine-or-repair theorems apply between maintenance events; merge, eviction, and compression may destroy information and must be evaluated as resource-management approximations.

\subsection{Closed-form local repair}

Ridge memory is the main closed-form adapter used in our checks. Let $X_{\mathrm{sup}}\in\R^{n\times d}$ be normalized support embeddings stored by a CGA memory and let $Y\in\R^{n\times C}$ be one-hot consequences. The adapter computes the dual ridge/Tikhonov solution \citep{tikhonov1977solutions,hoerl1970ridge}
\begin{align*}
\Gamma=(X_{\mathrm{sup}}X_{\mathrm{sup}}^\top+\lambda I)^{-1}Y,
\qquad
\operatorname{logits}(z)=zX_{\mathrm{sup}}^\top\Gamma.
