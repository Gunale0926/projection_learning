...
\end{align*}
For a block of length $L$, the DGM state history need not be stored for backpropagation; the additional activation memory is the selected read computation, not the full sequence of memory states. The DGM-Embedding layer replaces the $\abs{N}dr$ adapter term by the selected embedding-logit cost.
\end{theorem}
