...
\end{align*}
For a block of length $L$, the CA state history need not be stored for backpropagation in detached sparse-adapter variants; the additional activation memory is the selected read computation, not the full sequence of memory states. This is not the large-model CALM implementation used in the main experiments. The implemented CALM mixer uses a dense static concept bank with learned addresses, differentiable concept read/write weights, gated value writes, and an exact chunked concept-memory scan.
\end{theorem}
