...
DGM uses refinement and repair in the same runtime object. Distinction Learning asks what the learner can tell apart. Projection Learning assumes such a representation and asks how a state inside it should be minimally repaired. This section makes the relation formal in the finite one-hot case.

\subsection{One-hot partition memory}

Let $\calP=\{B_1,\ldots,B_n\}$ and let $\phi_\calP$ be the one-hot feature map from Section~\ref{sec:preliminaries}. A memory matrix $W\in\R^{m\times n}$ defines a value map
\begin{align*}
...
\begin{theorem}[One-hot memory is partition-measurable, informal version of Theorem~\ref{thm:one-hot-measurable:formal}]
...
\end{align*}
Distinction refinement changes the sigma-algebra $\calF_t$. Projection repair changes a state $s_t$ inside a fixed information geometry. Gradient descent is the linearized $\ell_2$ special case of soft projection. Backpropagation is the mechanism that computes such gradients in differentiable static graphs.
