...
CA uses refinement and repair in the same runtime object. Distinction learning asks what the learner can tell apart. Projection learning assumes such a representation and asks how a state inside it should be minimally repaired. The repair side is related to Bregman projection, Moore-Penrose projection, ridge/Tikhonov repair, passive-aggressive learning, online regression, and fast-weight memory updates \citep{bregman1967relaxation,penrose1955generalized,tikhonov1977solutions,hoerl1970ridge,crammer2006online,vovk1997competitive,ba2016using,schlag2021linear}. This section makes the relation formal in the finite one-hot case.
...
\begin{theorem}[One-hot partition memory, informal version of Theorem~\ref{thm:one-hot-measurable:formal}]
...
We take $K$ to be the matrix whose columns are stored keys, so $KK^\dagger$ is the orthogonal projector onto their span. For old key matrices $K$, a new key $k$ has CPM innovation
\begin{align*}
z=(I-KK^\dagger)k.
\end{align*}
...
\begin{theorem}[CPM conflict equals missing distinction, informal version of Theorem~\ref{thm:cpm-conflict-distinction:formal}]
