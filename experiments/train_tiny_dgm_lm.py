...
class DGMMixer(nn.Module):
    """Causal DGM sequence mixer.

    This module is the attention-replacement version of DGM.  It maps
    ...
    def __init__(
        ...
        *,
        ...
        self.dropout = nn.Dropout(dropout)
        self.last_stats = DGMForwardStats()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        stats = DGMForwardStats(tokens=batch * time_steps)
            ...
            for t in range(time_steps):
                ...
                if not values:
                    ...
                    if self.use_edges and edges:
                        eligible = []
                        for concept_id in top_ids.tolist():
                            ok = True
                    ...
                    value_tensor = torch.stack([values[int(i)] for i in top_ids.tolist()], dim=0)
                    z_t = (weights.to(value_tensor.dtype).unsqueeze(-1) * value_tensor).sum(dim=0)
                    best_pos = int(torch.argmax(weights).item())
                    selected = int(top_ids[best_pos].item())
    ...
    def __init__(self, args: argparse.Namespace, mixer_kind: str) -> None:
        ...
        if mixer_kind == "attention":
            ...
            self.mixer = DGMMixer(
                ...
                centroid_lr=args.dgm_centroid_lr,
                create_threshold=args.dgm_create_threshold,
                refine_threshold=args.dgm_refine_threshold,
                use_edges=args.dgm_use_edges,
...
class TinyLM(nn.Module):
    ...
    def dgm_stats(self) -> dict[str, float]:
        stats = []
        for block in self.blocks:
            if isinstance(block.mixer, DGMMixer):
...
def evaluate(model: TinyLM, tokens: torch.Tensor, args: argparse.Namespace, device: torch.device) -> dict[str, float]:
    ...
    out.update({f"dgm_{k}": v for k, v in model.dgm_stats().items()})
    return out
...
def train_one_model(
    ...
    for step in range(1, args.train_steps + 1):
        ...
        if step % args.log_every == 0 or step == 1:
            ...
            print(
                f"[tiny-dgm-lm] model={name} step={step}/{args.train_steps} "
    ...
    if args.save_checkpoints:
        ckpt_dir = args.output.parent / "tiny_dgm_lm_checkpoints"
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("experiments/results_tiny_dgm_lm.json"))
    parser.add_argument("--models", default="transformer,dgm")
    ...
    parser.add_argument("--dgm-state-dim", type=int, default=128)
    parser.add_argument("--dgm-max-concepts", type=int, default=64)
    parser.add_argument("--dgm-top-k", type=int, default=8)
    parser.add_argument("--dgm-temperature", type=float, default=0.2)
    parser.add_argument("--dgm-centroid-lr", type=float, default=0.05)
    parser.add_argument("--dgm-create-threshold", type=float, default=0.15)
    parser.add_argument("--dgm-refine-threshold", type=float, default=0.85)
    parser.add_argument("--dgm-use-edges", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--dgm-max-incident-edges", type=int, default=32)
    ...
    print(f"[tiny-dgm-lm] device={device} dtype={dtype}", flush=True)
    ...
    print("[tiny-dgm-lm] loading tokens", flush=True)
    ...
    print(f"[tiny-dgm-lm] train_tokens={train_tokens.numel()} val_tokens={val_tokens.numel()} vocab={vocab_size}", flush=True)
    ...
    result = {
        "protocol": (
            ...
            "TinyDGM-LM replaces the attention mixer in each decoder block with a causal DGM-Mixer that reads a "
    ...
    print(f"[tiny-dgm-lm] wrote {args.output}", flush=True)
